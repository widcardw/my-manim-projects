...
class TripleScene(Scene):
    ...
    def construct(self):
        ...
        self.play(
            FadeOut(circle_coin),
            FadeOut(circle_favo),
            Flash(coin.get_center(), color=PINK,
                  line_length=0.7, flash_radius=1.5),
            Flash(favo.get_center(), color=PINK,
                  line_length=0.7, flash_radius=1.5),
            Flash(good.get_center(), color=PINK,
                  line_length=0.7, flash_radius=1.5),
    ...
    def get_svg(self):
        self.good = SVGMobject(self.good_file).set_height(
            1).move_to(LEFT*2.5+DOWN*2.7)
        self.coin = SVGMobject(self.coin_file).set_height(1).move_to(DOWN*2.7)
        self.favo = SVGMobject(self.favo_file).set_height(
            1).move_to(RIGHT*2.5+DOWN*2.7)
