"""
关于 logo 创意：
	1.  基础元素为 M 和 K 的负空间设计
	2.  白色部分创意来自弦图
	3.  整体图案本身可生成一个不错的分形
	4.  配色致敬 3B1B（具体的蓝色和棕色还得再微调一下）
logo 主要创意由 @GrakePCH 提供，@GZTime、@cigar666、@鹤翔万里都提供了不少宝贵意见。目前设计工作还在继续完善，希望大家多提意见和建议
"""
...
class Logo(VGroup):
    ...
    def create_logo(self):
        ...
        p1 = Polygon(ORIGIN, RIGHT, 2 * UP,
                     stroke_width=0).set_fill(self.color_1[0], 1)
        p2 = Polygon(1.5 * RIGHT, 3 * UR, 3 * UP,
                     stroke_width=0).set_fill(self.color_1[1], 1)
        p3 = Polygon(2 * RIGHT, 3 * RIGHT, 3 * RIGHT + 2 * UP,
                     stroke_width=0).set_fill(self.color_1[2], 1)
        if not self.black_bg:
            p1.set_fill(self.color_3[0], 1), p2.set_fill(
                self.color_3[1], 1), p3.set_fill(self.color_3[2], 1)

        self.bg = Square(stroke_width=0, fill_color=BLACK if self.black_bg else WHITE,
                         fill_opacity=1).set_height(self.size * 2.5)
        if self.add_bg_square:
            self.add(self.bg)

        self.part_ur = VGroup(p1, p2, p3).move_to(
            [2.5, 1., 0] + self.shift_out)
        ...
        if self.black_bg:
            self.part_ur[0].set_fill(self.color_2[0], 1), self.part_ur[1].set_fill(
                self.color_2[1], 1), self.part_ur[2].set_fill(self.color_2[2], 1)
        else:
            self.part_ur[0].set_fill(self.color_4[0], 1), self.part_ur[1].set_fill(
                self.color_4[1], 1), self.part_ur[2].set_fill(self.color_4[2], 1)

        self.inner_triangles = VGroup(
            self.part_ur[0], self.part_ul[0], self.part_dl[0], self.part_dr[0])
        self.mid_triangles = VGroup(
            self.part_ur[1], self.part_ul[1], self.part_dl[1], self.part_dr[1])
        self.outer_triangles = VGroup(
            self.part_ur[2], self.part_ul[2], self.part_dl[2], self.part_dr[2])
...
class Logo_image(Scene):
    ...
    def construct(self):
        ...
        logo_white_bg = Logo(size=4.5, black_bg=False,
                             add_bg_square=True).shift(RIGHT * 3)
...
class Logo_01(Scene):
    ...
    def construct(self):
        ...
        square = VGroup(*[Polygon(ORIGIN, UR, UL), Polygon(ORIGIN, UL, DL),
                        Polygon(ORIGIN, DL, DR), Polygon(ORIGIN, DR, UR), ])
        square.set_fill(WHITE, 1).set_stroke(width=0.5, color=WHITE).rotate(
            np.arctan(0.5)).set_height(logo.inner_triangles.get_height())

        self.add(square)
        self.wait(0.5)
        self.play(ReplacementTransform(
            square, logo.inner_triangles), run_time=1.5)
        ...
        big_black_rect = Rectangle(stroke_width=0, fill_color=BLACK, fill_opacity=1).scale(
            100).align_to(LEFT * 1.4, RIGHT)
        big_black_rect_02 = big_black_rect.copy()
        big_black_rect_02.add_updater(
            lambda b: b.align_to(logo, RIGHT).shift(RIGHT * 0.15))
        text_font = 'Orbitron Bold'
        text_manim = Text('Manim', font=text_font, size=2.3).align_to(
            LEFT * 1.4, LEFT).align_to(logo.part_ur, DOWN)
        text_manim.set_color_by_text('M', logo.color_2[2])
        text_kindergarten = Text('Kindergarten', font=text_font, size=2.3).align_to(
            logo.part_dr, UP).align_to(text_manim, LEFT)
        text_kindergarten.set_color_by_text('K', logo.color_1[2])
        text = VGroup(text_manim, text_kindergarten).shift(
            LEFT * 8)
...
class Logo_02(Scene):
    ...
    def construct(self):
        ...
        line = Line(UP, DOWN, stroke_width=8, color=BLACK).move_to(
            mid(logo.get_right(), text.get_left()))
        line.set_length(1.4)
        text.add(line)

        bg = Rectangle(height=10, width=10, fill_color=BLACK,
                       fill_opacity=1, stroke_width=0)
        bg.add_updater(lambda m: m.move_to(
            logo, aligned_edge=RIGHT).shift(RIGHT*0.2))
        ...
        self.wait(0.25)
        for i in range(3):
            self.play(MyTransform(
                logo[i], logo[i+1], about_point=logo.get_center()), run_time=0.2, rate_func=smooth)
        self.wait(0.5)
        self.play(
    ...
    def construct(self):
        ...
        for i in range(5):
            ...
            for m in to_be_replaced:
                to_replace_m = logo.copy().move_to(
                    m.get_center()).set_height(m.get_height() * 1.335)
                if time_01/n < 1/self.camera.frame_rate:
                    ...
                    self.play(ReplacementTransform(
                        m, to_replace_m), run_time=time_01/n)
...
class MyTransform(Animation):
    ...
    def interpolate_mobject(self, alpha):
        ...
        for i in range(3):
            now[i].set_color(interpolate_color(
                self.starting_mobject[i].get_color(), self.target[i].get_color(), alpha))
...
class Logo_black(Scene):
    ...
    def construct(self):
        ...
        logo = Logo(size=8/3)
        squares = VGroup(*[Polygon(ORIGIN, UR, UL), Polygon(ORIGIN, UL, DL),
                         Polygon(ORIGIN, DL, DR), Polygon(ORIGIN, DR, UR), ])
        squares.set_fill(WHITE, 1).set_stroke(width=0.5, color=WHITE).rotate(
            np.arctan(0.5)).set_height(logo.inner_triangles.get_height())
        for s in squares:
            s.scale(0.8)
        ...
        line = Line(UP, DOWN, stroke_width=8).move_to(
            mid(logo.get_right(), text.get_left()))
        line.set_length(1.4)
        text.add(line)
        ...
        self.wait(0.2)
        self.play(ShowSubmobjectsOneByOne(tris),
...
class Logo_white(Scene):
    ...
    def construct(self):
        ...
        squares = VGroup(*[Polygon(ORIGIN, UR, UL), Polygon(ORIGIN, UL, DL),
                         Polygon(ORIGIN, DL, DR), Polygon(ORIGIN, DR, UR), ])
        squares.set_fill(BLUE_C, 1).set_stroke(width=0.5, color=BLUE_C).rotate(
            np.arctan(0.5)).set_height(logo.inner_triangles.get_height())
        squares[0].set_fill('#C59978', 1).set_stroke(
            width=0.5, color='#C59978')
        for s in squares:
            s.scale(0.8)
        ...
        bg = Rectangle(height=10, width=10, fill_color=WHITE,
                       fill_opacity=1, stroke_width=0)
        bg.add_updater(lambda m: m.move_to(
            logo, aligned_edge=RIGHT).shift(RIGHT*0.2))
...
class Logo_Rotate_Out(Scene):
    ...
    def construct(self):
        ...
        text = VGroup(
            VGroup(*[Text(t, font=self.font) for t in 'Manim']
                   ).arrange(direction=RIGHT * 0.5, aligned_edge=DOWN),
            VGroup(*[Text(t, font=self.font) for t in 'Kindergarten']
                   ).arrange(direction=RIGHT * 0.5, aligned_edge=DOWN)
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.3).set_height(2.1).next_to(logo, buff=1.5).shift(DOWN*0.2)
        ...
        def rotate_out(a, dt):
            ...
            if a.get_center()[0] < s.get_value() or a.get_center()[-1] != 0:
                a.rotate(w * (1 + 2.5 * np.random.random()) * DEGREES,
                         axis=UR, about_point=RIGHT * s.get_value() + OUT * 0.25)
