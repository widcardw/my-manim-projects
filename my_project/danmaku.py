    ...
    def construct(self):
        ...
        for f in range(2000):
            ...
            for t in range(min(f, 180)):
                ...
                dotg_i.add(dot_j)
            self.play(Transform(dotg_0, dotg_i), run_time=1/60)
            vg_rm = VGroup()
...
class Test2(Scene):
    def construct(self):
        ...
        dot_0 = Dot(myfunc(v, 1, f, a), color=RED).scale(0.5)
        self.add(dot_0)
        for f in range(0, 5000, 3):
            dot_i_group = VGroup()
            for t in range(min(f, 490)):
                dot_i = Dot(myfunc(v, t, f, a)).scale(0.5).set_color(
                    color_map[math.floor(t/20) % len(color_map)])
