...
class Line_tutorial(Scene):
    ...
    def construct(self):
        ...
        text = VGroup(*[CodeLine(obj, size=0.32, font="思源黑体 CN Bold")
        ...
        dot_0_0 = CodeLine(
            "(-4,-3)", size=0.3).next_to(l_2.get_start(), DOWN, buff=0.1)
        d_0_0 = Dot(np.array([-4, -3, 0]), color=RED)
        dot_0_1 = CodeLine("(0,3)", size=0.3).next_to(
            l_2.get_end(), UP, buff=0.1)
        ...
        self.wait(2.5)
        rect_0 = Rectangle(width=3.6, height=0.3, color=YELLOW,
                           fill_color=YELLOW, fill_opacity=0.9, plot_depth=-0.5)
        rect_0.align_to(code[2], DR)
        self.play(RT(text[3], text[4]), FadeOut(
            code[1]), FadeIn(code[2]), FadeIn(rect_0))
        l_2.add_updater(lambda l: l.become(
            Line(np.array([-4, -3, 0]), np.array([0, 3, 0]),
                 color=GOLD, buff=buff_value.get_value())
        ))
        ...
        l_2.add_updater(lambda l: l.become(
            Line(np.array([-4, -3, 0]), np.array([0, 3, 0]),
                 color=GOLD, stroke_width=stroke_value.get_value())
        ))
...
class Line_tutorial_2(Scene):
    ...
    def construct(self):
      ...
      path_arc=None)""",
        ...
        text = VGroup(*[CodeLine(obj, font="思源黑体 CN Bold")
                        for obj in text_list]).to_edge(DOWN*1.2)
        ...
        self.wait(2)
        l_3 = DashedLine(np.array([-2, -2, 0]),
                         np.array([0, 2, 0]), color=PURPLE)
        ...
        y_bg = Rectangle(height=0.3, width=3.6, plot_depth=-0.5,
                         color=YELLOW, fill_opacity=1).align_to(code[4], DR)
      ...
      path_arc=%.2f)""" % arc_fac.get_value()).next_to(code[3], DOWN, aligned_edge=LEFT)
        ...
        l_4.add_updater(lambda l: l.become(
            Line(np.array([-2, -2, 0]), np.array([-2, 2, 0]),
                 color=RED_D, path_arc=arc_fac.get_value())
        ))
        self.add(code[4], l_4)
...
class ArrowTutorial(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(axes, lag_ratio=0.1),
                  Write(text[0]))
        self.wait(2.5)
        l_1 = Line(np.array([-4, -2, 0]),
                   np.array([-4, 2, 0]), color=GOLD).add_tip()
        ...
        self.wait(2)
        y_bg = Rectangle(height=0.3, width=1.8, color=YELLOW,
                         fill_opacity=1, plot_depth=-0.5)
        ...
        vec_1.add_updater(lambda v: v.become(
            Arrow(np.array([-2, -2, 0]), np.array([-2, 2, 0]),
                  color=GOLD, buff=buf_fac.get_value())
        ))
...
max_stroke_width_to_length_ratio=%.1f)""" % buf_fac.get_value()).next_to(code[1], DOWN, aligned_edge=LEFT)))
        ...
        self.play(*[FadeOut(obj)
                    for obj in [text[6], code, l_1, vec_1, y_bg, text_l, text_v]])
