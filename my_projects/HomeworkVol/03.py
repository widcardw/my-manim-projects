...
class Test10(Scene):
    ...
    def construct(self):
        l_x = Line(LEFT*3, RIGHT*3, color="#559944")
        l_y = Line(UP*3, DOWN*3, color="#559944")
        self.play(ShowCreation(l_x), ShowCreation(l_y))
        t = ValueTracker(PI/2)
        dotq = Dot(color="#9988ff", background_stroke_color="#ffffff",
                   background_stroke_width=3, plot_depth=2)\
            .add_updater(lambda d: d.move_to(np.array([
                0, 3*np.sin(t.get_value()), 0
            ])))
        dotp = Dot(color="#9988ff", background_stroke_color="#ffffff",
                   background_stroke_width=3, plot_depth=2)\
            .add_updater(lambda d: d.move_to(np.array([
                3*np.cos(t.get_value()), 0, 0
            ])))
        l_pq = Line(color="#dd7766").add_updater(lambda l: l.put_start_and_end_on(
            dotp.get_center(), dotq.get_center()
        ))
        self.play(ShowCreation(dotq), ShowCreation(dotp), ShowCreation(l_pq))
        self.wait(0.5)
        self.play(t.increment_value, -TAU, run_time=6, rate_func=linear)
        self.play(t.increment_value, -TAU/160*8/6,
                  run_time=8/160, rate_func=linear)
        l_pq.clear_updaters()
        l_ = l_pq.copy().set_stroke(width=1.5)
        self.add(l_)
        l_pq.add_updater(lambda l: l.put_start_and_end_on(
            dotp.get_center(), dotq.get_center()
        ))
        self.add(l_pq)
        for i in range(80):
            if i != 79:
                self.play(t.increment_value, -TAU/80,
                          run_time=6/80, rate_func=linear)
                l_pq.clear_updaters()
                l_ = l_pq.copy().set_stroke(width=1.5)
                self.add(l_)
                l_pq.add_updater(lambda l: l.put_start_and_end_on(
                ...
                self.play(t.increment_value, -TAU/160*4/6,
                          run_time=4/160, rate_func=linear)
        l_pq.clear_updaters()
        dotp.clear_updaters()
        dotq.clear_updaters()
        # l_xx = Line(LEFT*3, RIGHT*3, color="#dd7766", stroke_width=1.5)
        # l_yy = Line(UP*3, DOWN*3, color="#dd7766", stroke_width=1.5)
        self.wait()
        #self.add(l_xx, l_yy)
        self.play(*[FadeOut(obj) for obj in [l_pq, dotp, dotq, l_x, l_y]])
        self.wait(2)
