...
class CurveBooleanOperations:
    ...
    def Union_Curve_and_Polygon(self, cur, pol):
        return VGroup(cur.copy(), pol.copy())
...
class PolygonBooleanOperations:
    ...
    def Intersection_Polygon_and_Polygon(self, pol1, pol2):
        pol1, pol2 = pol1.copy().stretch(0.999, 0), pol2.copy().stretch(0.999, 1)
    ...
    def Union_Polygon_and_Polygon(self, pol1, pol2):
        pol1, pol2 = pol1.copy(), pol2.copy().stretch(0.999, 0).stretch(0.999, 1)
        # 保证多边形都是逆时针
        if get_polygon_directed_area(pol1) < 0:
            pol1 = Polygon(*pol1.get_vertices()[::-1])
        if get_polygon_directed_area(pol2) < 0:
...
class PolygonWithHolesBooleanOperations:
    ...
    def Intersection_PolygonWithHoles_and_Polygon(self, pwh, pol):
        ## 可恶，好难啊
        pwh, pol = pwh.copy().stretch(0.9993, 0), pol.copy().stretch(0.9965, 1)
        # 拿点集
        pwh_vs = pwh.get_vertices()
        # 拿壳
        hull = Polygon(*pwh_vs[0])
    ...
    def Intersection_PolygonWithHoles_and_PolygonWithHoles(self, pwh1, pwh2):
        ## 这个超级难的，可恶，救命啊啊啊
        pwh1, pwh2 = pwh1.copy().stretch(0.9953, 1), pwh2.copy().stretch(0.9984, 0)
        # 拿点集
    ...
    def Union_PolygonWithHoles_and_Polygon(self, pwh, pol):
        ...
        pwh, pol = pwh.copy().stretch(0.9987, 0), pol.copy().stretch(0.9962, 1)
        # 保证多边形pol逆时针（大于0）
    ...
    def Union_PolygonWithHoles_and_PolygonWithHoles(self, pwh1, pwh2):
        ## 这个超级难的，可恶极了
        pwh1, pwh2 = pwh1.copy().stretch(0.9916, 1), pwh2.copy().stretch(0.9974, 0)
        # 拿点集
        pwh1_vs, pwh2_vs = pwh1.get_vertices(), pwh2.get_vertices()
        # 拿壳
        hull1, hull2 = Polygon(*pwh1_vs[0]), Polygon(*pwh2_vs[0])
    ...
    def Subtraction_PolygonWithHoles_and_Polygon(self, pwh, pol):
        ## 加油，可恶，这个简单极了
        pwh, pol = pwh.copy(), pol.copy().stretch(0.9927, 0).stretch(0.9992, 1)
        # 保证多边形pol逆时针（大于0）
        if get_polygon_directed_area(pol) < 0:
            pol = Polygon(*pol.get_vertices()[::-1])
        # 拿点集
    ...
    def Subtraction_Polygon_and_PolygonWithHoles(self, pol, pwh):
        ## 加油，可恶，这个简单极了
        pwh, pol = pwh.copy(), pol.copy().stretch(0.9996, 0).stretch(0.9935, 1)
        # 保证多边形pol逆时针（大于0）
        if get_polygon_directed_area(pol) < 0:
            pol = Polygon(*pol.get_vertices()[::-1])
        # 拿点集
    ...
    def Subtraction_PolygonWithHoles_and_PolygonWithHoles(self, pwh1, pwh2):
        ...
        pwh1, pwh2 = pwh1.copy().stretch(0.998, 1), pwh2.copy()
        # 拿点集
        pwh1_vs, pwh2_vs = pwh1.get_vertices(), pwh2.get_vertices()
        # 拿壳
        hull1, hull2 = Polygon(*pwh1_vs[0]), Polygon(*pwh2_vs[0])
...
class VGroupBooleanOperations:
    ...
    def Union(self, vgp1, vgp2):
        return VGroup(*get_all_not_VGroup_VMobject([vgp1, vgp2]))
    ...
    def Subtraction(self, vgp1, vgp2):
        ...
        for mob1 in mob1s:
            ismob
            for mob2 in mob2s:
                ismob = PolygonSubtraction(ismob, mob2)
                if type(ismob) != VMobject:
                    cutgps.append(ismob)
        if len(cutgps) == 0:
            return VMobject()
...
class PolygonBooleanTest(Scene):
    def construct(self):
        ...
        pol2 = Circle()
        ispol = VMobject().add_updater(lambda t: t.become(
            PolygonIntersection(pol1, pol2).set_stroke(color=YELLOW, width=0).set_fill(color=YELLOW, opacity=0.5)
        ))
        self.add(pol1, pol2, ispol)
        self.wait()
        self.play(pol2.shift, UR * 2, run_time=5)
        self.play(pol2.to_corner, DL, run_time=5)
        self.wait(2)
